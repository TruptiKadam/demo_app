class LibrariesController < ApplicationController
  ...
  def index
    @libraries = Library.includes(books: [:bookings, :users])
    data = {libraries: []}
    @libraries.each do |library|
      sub_data = { library: library }
      books = []
      library.books.each do |book|
        books << {name: book.name, remaining_quantity: book.remaining_quantity, total_quantity: book.quantity, users_who_brrows: book.users }
      end
      data[:libraries] << { library: library, books: books}
    end
    render json: data
  end
